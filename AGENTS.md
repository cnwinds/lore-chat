...
**根因层：** 记忆对象是主人稳定画像，不是对话里出现的世界知识，也不是助手的作业家规。错抽「上海卷 660 分制」的本质不是「分数」，而是**与主人无关的常识被当成了画像**。错抽「以后写可复用包 / 写库都要怎样」的本质不是某个产品名，而是**规范助手怎么做事的规矩被当成了主人是谁**。

**判定口诀（应写进抽取 prompt）**

> 删掉这句话，主人的长期画像是否变少？若不变，就不要记。
>
> 这句话规范的是助手怎么做事，还是主人是谁？前者不要记（应写《戒律》）。
...
- 把常识改写成第一人称伪自述
- 规范助手怎么做事的家规（写库、检索、生成可复用包等），即使用户说「以后都要」
...
| 以后写可复用包 / 写库都要怎样（助手作业规矩） | 否 | 助手怎么做事 ≠ 主人是谁 |
...
- **禁止**用关键词黑名单代替上述原则（如禁止记某课程名、某依赖名、某产品能力名）
- **禁止**只修某一个错例的 prompt 句子，而不提升到根因原则（见 §0）
- 修改记忆抽取 prompt 后，验收意图须覆盖「常识提及 ≠ 画像事实」「任务提及 ≠ 画像事实」「去语境泛化 ≠ 完整画像事实」「助手家规 ≠ 主人画像」这类，而非单句回归
...
- [ ] 与代码硬约束无重复、无矛盾
- [ ] 若动记忆抽取：已体现关于主人（含「助手怎么做事 ≠ 主人是谁」）/ 耐久性 / 语境保全门槛与口诀
