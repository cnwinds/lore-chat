...
- 界面更紧凑：浮窗/弹窗头栏只留大标题，去掉小标签和说明；设置分组不再在标题下复述用途。左栏「知识库」与聊天输入框之间留出空隙，目录也不贴边；拖动缝静息不画线，悬停才是细线。滚动条改为隐藏式，不占布局。左下角「主题 / 聊天通道 / 设置」始终留在栏内
...
- 文档专注阅读时中栏与左栏贴齐，不再留一条空缝

### Added

- PC 上助手落款的模型名后用括号标本轮用量，形如 `(in:100·out:5000)`；手机不显示
