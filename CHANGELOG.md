...
- 聊天通道改为左栏底栏悬浮窗：底栏恢复「主题 | 聊天通道 | 设置」；点「聊天通道」在底栏上方弹出浮层，聊天区不被挤开。通道卡片直接露出接入说明 / 会话 / 日志 / 吊销页签，默认收起内容，再点同一页签可折上；不再套「详情」手风琴
...
- 助手把征询写成正文（例如【征询】+ 选项）时，会提升为可点选的提问卡片；群聊与私聊同一条路径。需要用户做选择时须调用 `ask_user`，正文不会出现按钮
