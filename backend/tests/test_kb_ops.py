...
def test_import_md_and_attachment(tmp_path):
    ...
    r1 = import_file(
        w, directory="技术", filename="note.md", data=b"# Hi\nbody\n"
    )
    assert r1["kind"] == "markdown"
    assert repo.read_doc("技术/note.md").body.strip().startswith("# Hi")

    r2 = import_file(
        w, directory="技术", filename="plan.pdf", data=b"%PDF fake"
    )
...
def test_import_conflict(tmp_path):
    ...
    w = _writer(repo, tmp_path)
    import_file(w, directory="", filename="a.md", data=b"x\n")
    with pytest.raises(KbPathExistsError):
        import_file(w, directory="", filename="a.md", data=b"y\n")
...
def test_move_attachment(tmp_path):
    repo = KnowledgeRepo(tmp_path / "knowledge")
    w = _writer(repo, tmp_path)
    import_file(w, directory="a", filename="f.txt", data=b"hello")
    new = move_entry(
        w, from_path="a/attachments/f.txt", to_directory="b", to_filename="f.txt"
    )
...
def test_delete_attachment(tmp_path):
    repo = KnowledgeRepo(tmp_path / "knowledge")
    w = _writer(repo, tmp_path)
    import_file(w, directory="x", filename="z.bin", data=b"\x00")
    deleted = delete_entry(w, "x/attachments/z.bin")
