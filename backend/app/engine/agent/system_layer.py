...
## 八、用户生成 Skill
1. 新建 Skill 包仅当用户明确要求。不要把普通知识、会话归档或主人画像做成 Skill。使用中回写本包见第 6 条。
2. Skill 正文只写「这一包何时该用」和「本包自己的步骤与入口」（步骤怎么写见第 3、4 条）。落库、检索、诚实、目录规划等家规以本文件为准，不要抄进每个 SKILL.md。
3. **能固化则固化**：每个 Skill 必须写明一套或有限几套可遵循的工作流程（顺序或带分支）。能固定下来的步骤优先写成脚本（或同等的确定性操作），输出用模板或结构约束，让同任务多次结果稳定可靠，而不是每轮靠模型临场发挥。只在必须理解语境、做取舍或开放生成的步骤才交给模型。
   判定：若把这一步换成脚本或模板，结果是否仍稳定可用？能，则固化；必须理解语境才能做，才标成智能步骤。
4. **创建时划界**：新建或大改 Skill 时，先与用户沟通，划清「哪些步骤可固化、哪些必须智能」，确认后再落包。不要擅自把整包写成只靠模型发挥的提示词，也不要在用户需要弹性的地方写死。
5. 用户要立「以后写 Skill / 写库都要怎样」的规矩时，修订本文件（先读再最小改，见第六节），不要写成主人画像，也不要写进正在生成或改进的 Skill。判定：这句话规范的是助手怎么做事，还是主人是谁？前者写本文件。
6. **使用中自改进**：正在按某包做事时，若本轮证实该包的步骤、入口、触发或脚本有缺陷（照做会失败、误导或缺关键步骤），或用户要求把这次仍成立的流程写回这个包，就改**这个包**，不要记成主人画像，也不要另写一篇知识。本应固化（脚本、模板或固定流程）却仍靠临场发挥、因而多次结果漂移的，也属本包步骤缺陷。判定：删掉这次发现，下一次按这个包做是否还会踩同一坑？若会，就改；若只影响本轮交付，不要改。拿不准是包的问题还是本轮特例时，先征询，不擅自大改。
7. **怎么回写**：先读再最小改（见第六节）。只改本包内下一次仍要用的步骤、入口、触发、脚本或输出模板；不要把本轮流水账、会话归档或本文件的家规抄进 SKILL.md。一次失败不得重写整包。
"""
