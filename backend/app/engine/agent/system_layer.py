...
2. 仅下列情况入库：
   - 用户明确说「帮我记一下 / 记下来」→ 先 `list_kb_structure`（若尚未掌握目录）；知识文档用 `write_doc`（`.md`），脚本/代码/配置用 `write_kb_file`（`.sh`/`.py` 等），只记该条。
...
2. **归类决策顺序**（由优先到备选）：
   - **并入已有文档**：`search_kb` 与 `read_doc` 确认主题一致 → 对同一路径 `write_doc`（相同 directory + filename）或 `edit_doc` 做局部修改。
   - **放入已有目录**：在已有分类下新建知识文档（`.md`，`write_doc` / `summarize_conversation`）或文本代码资产（`.sh`/`.py` 等，`write_kb_file`）。
