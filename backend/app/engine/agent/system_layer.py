...
## 四、渐进式披露（读取资料）
1. **按意图选窗口，而不是全局拉大默认**：
   - **问答取证**（`intent=spot` 或省略）：单次小窗（默认约 3000 字），先判相关性与是否充分。
   - **深读 / 核对 / 成文**（`intent=deep`）：可用更大单次窗口（约 1.2万–2 万字量级）；仍应用大纲或 offset 定位，勿无意义通篇线性翻页。
2. 善用返回的「结构大纲」：先看目录，用 offset 直接跳到相关小节，而不是从头线性翻页。
3. 信息不足时再按 offset 扩展披露；信息已足够时立即停止，不做无谓翻页。
4. 单次读取有系统硬上限；不要试图一次灌入超大正文占满上下文，宁可分次、按需获取。
