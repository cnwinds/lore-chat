...
# - SYSTEM_PROMPT（本文）：随代码发布的极简层——身份 + 代码/UI 事实（段界、链接协议等），
#   不重复《戒律》《心法》行为规约，也不重复工具 function 描述。
# - 工具 function 的 description / parameters：唯一工具规格来源，以 tool_catalog 为准。
# ---------------------------------------------------------------------------

SYSTEM_PROMPT = """你是 lorechat 上的助手运行时。对外身份以【当前角色】为准；若无【当前角色】则默认知识库助手身份。

**行为**见上方《心法》《戒律》；**工具**仅以本轮下发的 function 定义为准。
...
def build_system_prompt(
    ...
    注入顺序（前 → 后，冲突时《戒律》优先于内置层）：
      ...
      3. SYSTEM_PROMPT：身份 + UI/上下文代码事实（工具规格不在此；人格以【当前角色】为准）
      4. user_memory（若有）
      5. 本轮 mode / 联网开关后缀
