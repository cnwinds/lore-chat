...
def utc_year(now: datetime | None = None) -> str:
...
def media_upload_dir(year: str | None = None) -> str:
    y = (year or utc_year()).strip()
    return f"{MEDIA_ROOT}/{MEDIA_UPLOADS}/{y}"


def media_generated_dir(year: str | None = None) -> str:
    y = (year or utc_year()).strip()
    return f"{MEDIA_ROOT}/{MEDIA_GENERATED}/{y}"
...
def rewrite_legacy_media_path(
    ...
    *,
    ...
    if is_legacy_generated_path(norm):
        ...
        return (
            f"{MEDIA_ROOT}/{MEDIA_GENERATED}/{rest}"
            if rest
            else f"{MEDIA_ROOT}/{MEDIA_GENERATED}"
        )
    if is_legacy_inbox_image_path(norm):
        name = PurePosixPath(norm).name
        # 无 mtime / path_map 时落到当前年；迁移应用 path_map 覆盖
