...
def _is_platform_path(rel: str, platform_dirs: tuple[str, ...]) -> bool:
...
def is_kb_empty(
    ...
    root = Path(kb_path)
    platform_dirs = (system_layer_dir, skills_dir)
    if _has_conversations(root):
        return False
    if _has_user_markdown(root, platform_dirs):
        return False
    if _has_user_files(root, platform_dirs):
        return False
    return True
...
def _has_user_markdown(root: Path, platform_dirs: tuple[str, ...]) -> bool:
    for path in sorted(root.rglob("*.md")):
        rel = path.relative_to(root).as_posix()
        if rel.startswith(".kb/"):
            continue
        if _is_platform_path(rel, platform_dirs):
            continue
        return True
    return False


def _has_user_files(root: Path, platform_dirs: tuple[str, ...]) -> bool:
    """非 Markdown 用户文件（任意后缀），不计 .kb/.git/系统层与技能目录。"""
