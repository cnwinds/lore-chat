...
| 聊天通道 | `backend/app/engine/channel_plugins/` | Registry / Adapter / 实例存储 / `ChannelTurnService`（验身份 → hidden role → `begin_persisted_turn`）；`ChannelRuntime` 挂飞书长连接；按实例日志与用量。HTTP **不**另写 Agent 编排。第一种类型 `script_api` 投影自 API Key；P1 类型 `feishu` 默认 websocket。见 [product-channel-plugins.md](docs/product-channel-plugins.md) |
