...
**界面简洁**：面板、浮窗、弹窗头栏只放一个标题。不要小标签叠大标题，也不要在标题下再跟操作说明、重复路径或同义 hint。区块标题下面不要写「控制某某 / 切换某某」这类复述。空位用留白，不要把图标拉变形去填满格子。左栏「知识库」标题与目录之间留一点呼吸，左右不要贴边；不要回到大块空白，也不要贴死。气泡里时间戳、「参考」用留白分开，不要再画一道横线。
_Avoid_: `.kb-float-kicker` + 标题 + `.kb-float-crumb` 说明三件套；群成员拼贴用跨行拉伸或 `object-fit: cover` 放大裁切；知识库标题下大块空白或与目录贴死；设置分组标题下的同义说明；`.chat-meta` / `.chat-sources` 顶部分隔线
