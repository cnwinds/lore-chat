...
| 聊天通道 | `backend/app/engine/channel_plugins/` | Registry / Adapter / 实例存储 / `ChannelTurnService`（验身份 → hidden role → `begin_persisted_turn`）；HTTP **不**另写 Agent 编排。第一种类型 `script_api` 投影自 API Key。见 [product-channel-plugins.md](docs/product-channel-plugins.md) |
