...
**用户生成 Skill 的家规**：何时写包、正文写什么、立规矩走《戒律》第八节；YAML `name`/`description` 在 `write_doc` 契约；本轮用哪个包看 `[Skill 目录]`；主人是谁才进记忆。
_Avoid_: 把作业家规写成主人画像；用 Skill 专名黑名单代替「助手怎么做事 ≠ 主人是谁」；在 SYSTEM_PROMPT 或每个 SKILL.md 复述落库/检索家规；在《戒律》堆 YAML 触发头 schema
