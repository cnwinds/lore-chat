...
_Avoid_: `.kb-float-kicker` + 标题 + `.kb-float-crumb` 说明三件套；群成员拼贴用跨行拉伸或 `object-fit: cover` 放大裁切；知识库标题下大块空白；设置分组标题下的同义说明；`.chat-meta` / `.chat-sources` 顶部分隔线；把拖动缝画成一段画布；左栏与聊天/文档/媒体/通道之间留接缝；常驻滚动条；手机提问用带字胶囊 FAB；群角色气泡左侧空一列给头像；通道/媒体头栏 ⟦⟧ 宽窄按钮；底栏三钮压成扁条；主题菜单 z-index 低于手机侧栏；信息流思考/工具/参考默认摊开；手机气泡落款下列 token；落款用量写成 `(in:n·out:n)`
