...
**界面简洁**：面板、浮窗、弹窗头栏只放一个标题。不要小标签叠大标题，也不要在标题下再跟操作说明、重复路径或同义 hint。区块标题下面不要写「控制某某 / 切换某某」这类复述。空位用留白，不要把图标拉变形去填满格子。左栏「知识库」标题与目录紧贴。气泡里时间戳、「参考」用留白分开，不要再画一道横线。折叠处用同一套细线箭头，靠旋转表达开合，不要用 ▸▾▶▼ 字符三角。
