...
现网对照（以代码为准）：`OpenApiService.complete_chat`（验 Key → 取 hidden `role_id` → `begin_persisted_turn`，`mode=api`，`origin=api`）；聊天通道入口在左栏底栏，侧栏右侧面板列出实例。现有密钥是列表里的第一种卡片。
...
聊天通道不在设置弹窗里。左栏底栏是浅色横条：**「聊天通道」|「设置」**。点「聊天通道」在侧栏右侧打开浅色面板，聊天主区保持可见。点「设置」仍打开设置弹窗；设置页签**不再包含**「聊天通道」。

### 5.1 入口

**已拍板：** 文案 **「聊天通道」**。不要「开放接口」作主名，**也不保留副名**。副标题：「每张卡片一条通道。密钥常驻可复制；详情用切页，默认收起。」不要只写「插件」。
...
**详情**默认收起，内用切页：**接入说明 | 会话 | 日志 | 吊销**（非脚本为 **删除**）。不要把接入说明做成面板底部的全局手风琴。
...
- **接入说明**：放进该实例详情切页。`script_api` 的 curl 与 IM 的回调步骤都在对应卡片里。

### 5.5 查看会话（共有）

在该实例详情切页「会话」里看只读记录。数据：该 hidden 角色的 timeline / 会话列表。两实例选同一人设：卡片上角色同名，点进去是**两份互不相干的聊天**。
