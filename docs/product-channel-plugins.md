...
现网对照（以代码为准）：`OpenApiService.complete_chat`（验 Key → 取 hidden `role_id` → `begin_persisted_turn`，`mode=api`，`origin=api`）；聊天通道入口在左栏底栏，点开后是底栏上方的悬浮窗列出实例。现有密钥是列表里的第一种卡片。
...
聊天通道不在设置弹窗里。左栏底栏是浅色横条：**「主题」|「聊天通道」|「设置」**（主题可以是图标按钮）。点「聊天通道」在底栏上方打开悬浮窗，聊天主区仍在下面、不被挤成侧栏。点「设置」仍打开设置弹窗；设置页签**不再包含**「聊天通道」。关闭：右上角 ×、Esc、点浮窗外。

### 5.1 入口

**已拍板：** 文案 **「聊天通道」**。不要「开放接口」作主名，**也不保留副名**。副标题：「悬浮窗 · 密钥可复制 · Tab 直接点开」。不要只写「插件」。
...
- 页签条：**接入说明 | 会话 | 日志 | 吊销**（非脚本为 **删除**）。默认只显示页签、不展开正文；点某一页签才展开，再点同一页签折上。不要再套一层「详情」手风琴。不要把接入说明做成面板底部的全局手风琴。
