...
| 角色归属 | **方案 A**：隐藏系统角色 `__api__`（不进左栏）。人设可在开放接口页填写；沙箱即该 slot。方案 B（借现有角色人设）留作后续，不改沙箱契约。 |
...
开放接口页可以写人设；真要借用某个角色口气，再加 B（Key 上一个可选 `persona_role_id`），沙箱契约不变。
...
沟通未回时按 **方案 A** 锁定。若之后要借现有角色口气，加 Key 级 `persona_role_id`（方案 B），沙箱仍只打 API slot。独立展示、只读 KB/Skill、专用沙箱不再改，除非明确推翻。
