...
产品对象是 **聊天通道插件**。现有 API Key 是第一种类型（`script_api` / HTTP），与飞书等**同级列表**。

共有：`persona_id`、hidden `role_id`、启停与状态、外部线程 → `conversation_id`、按实例只读时间线、（建议）日志/用量、`mode=api` 回合。
...
### 2. 人设可共享；不绑左栏 `role_id`

通道选的是 **说话方式（persona）**，不是左栏那个工作角色。从左栏创建时只**复制**名称/头像/提示词。默认不跟随左栏 `system_prompt`（与已确认「改左栏通用不影响 Key」一致）。直接挂左栏 `role_id` **否决**。

### 3. 每通道实例一个 hidden 工作角色

与「每把 Key 一个角色」同构。外部 `chat_id` / `thread` / `openid` 经查找表映射到 `conversation_id`。

### 4. IM 入站先 ACK，后台跑回合

Webhook 在厂商时限内 200。同角色忙：脚本仍 HTTP 409；其它类型 ACK 后入队，不对平台 409。

### 5. 全部聊天通道默认 `mode=api`

只读 KB + Skill + 沙箱；不写库、不回写、不改角色/例行、不派工。`origin`：`api` 留给脚本；IM 用类型名。左栏与记忆排除全部通道 origin。
...
P0 公共模型 + Key 投影为第一种通道 + 同级列表 UI，行为不变。P1 加一个免公网类型（建议飞书长连接或 Slack Socket Mode）。P2 其余类型、群、富媒体。

## 后果（若采纳）

- 设置页「开放接口」演进为「聊天通道」；脚本与 IM 同一卡片结构。
- `sandbox_max_api_roles` 计入所有 hidden 通道角色。
- 长连接 task 挂在 `Container` 生命周期。
