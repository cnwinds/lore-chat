...
> 状态：**已落地**（2026-09-12 拍板，P0–P2 已实现）。权威决策见 [ADR 2026-09-12](../adr/2026-09-12-role-rooms.md)。三点锁定：委托自动开干、协作上双方时间线、群是独立现场（左栏与角色混排，见 [ADR 2026-09-13 inbox](../adr/2026-09-13-group-inbox-mix.md)）。
>
> 配套现状：[ADR 2026-09-09 多角色底座](../adr/2026-09-09-multi-role-shell.md)、[ADR 2026-09-10 统一时间线](../adr/2026-09-10-role-timeline.md)、[ADR 2026-09-10 每角色沙箱](../adr/2026-09-10-role-scoped-sandbox.md)、[multi-role.md](multi-role.md)。
>
...
群是**独立现场**，像角色一样有名字和头像，不属于某一个角色。左栏**不单独开「群聊」分类**：群和角色按最近活动排在同一列表，群头像用圆角方和左上角「群」字区分（右下角留给在线/忙碌）。加号可以新建角色或发起群聊。

打开群，中栏就是正常的多人聊天：

- 主人靠右。每个角色靠左，**带头像和名字**。谁说的就是谁，不是「当前助手」一种气泡。
- 主人 `@游戏开发助手` 才叫醒对方。没点名就不自动全员抢麦。
- **例外**：有人刚用 `ask_user` 问了主人，点选或下一句（即使没 `@`）是在回答提问者，不是新的点名交棒。正文里再 `@` 提问者也不改协调者；只有 `@` 了别人，才是新指令。
...
一对一委托 = 两人房间 + 「另一方默认唤醒」。群 = 多人房间 + 「只有被点名的才唤醒」（未答征询除外）。没有第二套引擎。
...
- 角色时间线改为：**我是 participant 的 `owner_dm` + `peer_dm`**。群不并入全文；只投影 `kind=group_card`（自己在该群的最近一次应声）。可写 tip 仍只来自自己的 `owner_dm`。
- 群聊主入口是左栏「群聊」。群有 `avatar`（自定义或成员拼贴）。角色时间线上只挂参与卡片。
...
| `group` | 任何人 | **仅** `mentions`；没点名则不唤醒角色。**例外**：工人回执唤醒 assigner；逾期系统刺激唤醒协调者；未答的 `ask_user` 把主人下一句绑在提问者身上 |
| 任意 | 自己 | 永不唤醒自己 |
...
角色配置已经以工具为主要接口（见 [multi-role.md](multi-role.md) §3.3）。互通同样走工具，不另开一套 HTTP 编排。
...
| `create_group` | 建群：标题 + 头像 + 角色列表。主人自动是成员。 |
...
- 左栏「群聊」列表与角色一样有头像；可改名、换头像、圈人、删除。
- 选中群：中栏只显示该房间，按说话人画气泡；composer 是主人在对房间说话，`@` 出角色。
- 角色时间线遇到群只出参与卡片，不展开群 transcript。
...
3. **群聊入口？** 群不属于某一个角色，但是和角色混在同一对话列表里，靠头像群标记区分；不要再单独开一栏「群聊」。

P0–P2 已按本文与 ADR 落地。群作为独立现场的修订见 [ADR 2026-09-13](../adr/2026-09-13-group-as-stage.md)：头像与 CRUD、群内按说话人显示、角色时间线只出参与卡片。左栏混排见 [ADR 2026-09-13 inbox](../adr/2026-09-13-group-inbox-mix.md)。群头像标记见 [ADR 2026-09-13 mark](../adr/2026-09-13-group-avatar-mark.md)。
