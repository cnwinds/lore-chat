...
> 状态：**已确认 / P2 已落地**（2026-09-14）。主人已拍板 §12 全部 10 条。P0（公共模型 + 脚本通道 + 设置页）、P1（飞书长连接 + 按通道实例日志/用量）与 P2（Slack Socket Mode、企微 webhook、钉钉 Stream、群/@/thread、富媒体 attachments、`needs_input` 纯文本降级）已落地。`wechat_mp` 仅骨架灰显。不做：个微、插件商店、外部群接到左栏、实例级写库、独立人设、类型特有卡片。
>
...
共有：选说话方式、查看会话、启停与状态、会话映射、日志/用量（**P1** 按通道实例；P0 脚本沿用「上次调用」）。特有：Bearer vs 签名 vs 长连接、厂商字段、同步 JSON vs 异步 reply。
...
| 工作角色 | `role_id`（hidden，**每实例一个**） | 角色 id 前缀可不同（`api_` / `ext_`），规则相同 |
| 启停 | `enabled` | 启用时注册何种 ingress |
| 过程输出 | `show_thinking` / `show_tool_output`（默认关）。关则外部只收到终答与征询；开则把思考过程、工具结果写进出站文本 | 无。不要做成某 IM 特有开关 |
...
| 日志 / 用量 | **P1 共有**：入站失败、出站重试、回合耗时；用量页按通道实例过滤。P0 脚本只展示「上次调用」 | 厂商原始 payload 只进该实例日志，不上公共时间线 |
...
| 能力 | 行为 |
|------|------|
| **选择说话方式** | 绑定人设 `persona_id`。只绑 persona，不绑左栏 `role_id`（§2.5） |
...
| `show_thinking` / `show_tool_output` | 是否把思考过程、工具结果写入外部回复；**默认 `false`** |
| `persona_id` | 说话方式；活引用 |
...
| **左栏角色** | 人设 + 工作角色 + 网页时间线，给主人在三栏里聊 | **不绑**左栏 `role_id` | 是 |
...
4. 禁止请求或 IM 事件里带一个左栏 `role_id` 去借沙箱。
5. **不跟随**左栏角色的 `system_prompt`（方案 B 不采用）。「飞书上就是通用助手」只能通过复制人设，或共用一份独立 persona，不能活挂左栏那一行。

| 方案 | 含义 | 结论 |
|------|------|------|
| **A. 只绑人设** | 与现网 Key 相同；从左栏复制也是副本 | **已拍板** |
| B. 跟随左栏角色的人设 | 活引用某左栏 `system_prompt`，工作角色仍 hidden | **不采用**（与「改左栏不影响 Key」冲突） |
| C. 直接绑左栏 `role_id` | 外部消息进左栏时间线、共用那把沙箱 | **否决** |

删人设：仍有任一通道实例在用则拒绝（现网只查 Key，实现时查全部实例）。「改为独立人设」仍是对外聊天 API 的 P1，本文不挡。
...
| 策略 | 做法 | 利 | 弊 |
|------|------|----|----|
| **A. 每实例一个角色** | 启用时 `roles.create(visibility=hidden)`，如 `ext_<instance_id>` | 与 Key 同构；沙箱占用可预期 | 同一实例上多条外部会话**串行**（忙则排队） |
| B. 每个外部线程一个角色 | 新 `chat_id` 懒创建 hidden role | 多线程真并行 | `sandbox_max_api_roles`（默认 8）易满 |
| C. 全部通道共用一个角色 | 一个 `__im__` | 实现少 | **否决**：与已确认 Key 模型冲突 |

**已拍板：策略 A**（每实例一角色 + IM 入站排队）。不按外部线程开角色。`script_api` 继续 `id=api_<key短id>`。其它类型前缀 `ext_`，实现时把 `is_api_role_id` 收成「hidden 通道工作角色」（§11）。
...
| IM 私聊 | `type + instance_id + dm + 对方 id` | **每人一段会话** |
| IM 群 / 频道 | 仅 **被 @ 或被引用** 才开回合；会话键 `type + instance_id + chat_id + thread_id`（无 thread 则只用 chat_id） | 群与私聊绝不共用一段 |
| Slack 频道（及同类） | 无 @、无引用的裸消息 **忽略**。命中后 **回复开成 thread**，不要顶栏另刷一条 | 跨类型口径，P1 飞书以私聊为主，P2 群/频道按此做 |
...
2. 保存：公共必填 + 类型 schema。**允许先保存再启用。** webhook 类若缺 `public_base_url`：配置仍可落盘，勾选启用则校验失败 → `status=error`，卡片写明缺公网根；**不要**因缺公网拒绝保存（§12）。
3. 校验通过且 `enabled` 才注册 ingress。其它校验失败同样 `status=error`，卡片可见，**不**静默丢消息。
4. `script_api`：创建即启用；吊销 = `enabled=false`。长连接与 `script_api` **不**依赖公网。
...
| 同实例角色已有 running turn | 角色锁。IM **不要**对平台 409；ACK 后进**实例入站队列**。`script_api` 仍 HTTP 409 |
| 回合失败 | 可向该外部会话发一句失败说明；内部时间线仍能看 |
| 回合很长 | 内部跑完为止（与网页断开不取消执行同一套 hub）。**不要**先发「还在处理」 |
...
| 高风险 `sandbox_run` | 与脚本通道一样跳过网页确认（自用假设） | 实例开关「本通道禁用沙箱」；群聊默认关沙箱直到有发送者白名单 |

**已拍板：** P1 **允许**非脚本通道使用沙箱（`mode=api` 已含沙箱工具）。群聊默认关沙箱，直到有发送者白名单（P2 备注，不挡 P1 飞书私聊）。
...
与飞书同构（都能免公网）。**P1 不做 Slack**；P2 按 §2.7 群/频道口径加 Socket Mode 或 webhook。公共模型不变。
...
- webhook 类型：**允许保存配置**。缺 `public_base_url` 时若勾选启用，校验失败 → `status=error`，卡片写明缺公网根。**不要禁止保存。** 配好公网根后可再启用。

### 4.7 密钥存储

| 现网 | 做法 |
|------|------|
| API Key | SHA-256 落盘 `.kb/api_keys.json`；实例 `secrets.key_plaintext` 供主人随时复制（列表不回传；Cookie `GET .../credential`） |
| 模型 / 搜索 Key | `.kb/settings.json`；API 脱敏；PATCH 空/掩码保留旧值 |

IM 的 App Secret **运行时还要拿去调厂商**，不能只存哈希。

**实现约定：** `script_api` token 继续哈希，同时在实例 secrets 里保存一份明文供主人复制。其它通道 secret 对齐 settings：实例存储里落盘、回包脱敏、空/掩码不覆盖。**P0 不做**单独信封加密；要做就和 settings 一起做。旧投影 Key 若无明文，复制会提示需吊销后重建。

---

## 5. UI（左栏底栏入口 + 左缘浮窗）

聊天通道不在设置弹窗里。左栏底栏是浅色横条：**「主题」|「聊天通道」|「设置」**（主题可以是图标按钮）。点「聊天通道」打开与**媒体图库 / 记忆画像同一套**聊天区左缘浮窗（`KbFloatLayer`：`.doc-float-backdrop` + `.doc-float-panel`，内容 `.kb-float-panel`），与它们互斥同槽；右侧聊天仍可见，不挤开侧栏。点「设置」仍打开设置弹窗；设置页签**不再包含**「聊天通道」。关闭：右上角 ×、Esc、点浮窗外遮罩。

### 5.1 入口

**已拍板：** 文案 **「聊天通道」**。不要「开放接口」作主名，**也不保留副名**。头栏 kicker / 标题用「聊天通道」，副文案：「密钥可复制 · Tab 直接点开」。不要只写「插件」。
...
- 名称、类型 badge、启用滑动开关
- **启用时** Key / 凭证可一键复制（不必重新生成才能复制）。**脚本吊销 / 停用后不再展示密钥前缀，也不能复制**；打开开关可重新启用同一把 Key。IM 停用后凭证仍保留。
- 角色选择：共用角色或本通道专属；旁有编辑提示词
- **输出**：思考 / 工具两个滑动开关，默认关。只影响外部回复是否带上过程，不改变内部时间线
- 页签条：**接入说明 | 会话 | 日志 | 吊销**（非脚本为 **删除**）。默认只显示页签、不展开正文；点某一页签才展开，再点同一页签折上。不要再套一层「详情」手风琴。不要把接入说明做成面板底部的全局手风琴。非脚本 **删除须先停用**：仍启用时说明「请先停用」且按钮不可点；停用后才能从列表移除实例（历史保留）。脚本 **吊销** = `enabled=false`，卡片仍在。

底部 **共用角色** 默认收起，可建多套，通道卡片里任选。
...
3. **特有表单**：该类型 schema。
4. 保存并启用。`script_api` 的 Key 可从卡片随时复制（Cookie `GET .../credential`）；列表接口仍不回传明文。
5. IM：成功则展示该类型的下一步（开长连接权限、粘贴回调 URL）。

### 5.4 折叠区

- **共用角色**：面板底部，默认折叠；可建多套，通道卡片里任选。「N 个通道在用」。
- **接入说明**：放进该实例卡片页签。`script_api` 的 curl 与 IM 的回调步骤都在对应卡片里。

### 5.5 查看会话（共有）

在该实例卡片「会话」页签里看只读记录。数据：该 hidden 角色的 timeline / 会话列表。两实例选同一人设：卡片上角色同名，点进去是**两份互不相干的聊天**。
...
- 不要用「插件」当页签名。
- 不要把聊天通道再放回设置弹窗页签。
- 不要做成挡住聊天的整页模态框（用与媒体图库相同的左缘浮窗，右侧主区仍可见）。
- 不要另做一套底栏气泡或通道专用浮层皮肤。
- 不要用「详情」手风琴把页签再包一层。
...
**已拍板：所有聊天通道默认同一 `mode=api`。** 外部是半信任边界；与「脚本乱写知识库」同一类风险。P2 再做实例级「允许写库」（默认关）。

记忆：**脚本与其它通道 P1 都不跑抽取**（不 dirty）。左栏 tip / 记忆调度排除全部通道 origin。

沙箱：hidden 角色走 `sandbox_max_api_roles`，不占左栏名额。脚本与 IM 实例**合计**受该上限；满则本轮失败，**绝不借用**左栏或其它实例的盘。**P1 允许非脚本通道用沙箱。** 群聊默认关沙箱直到有发送者白名单（P2）。
...
**第一家 IM = 飞书 + 长连接。** 公共模型不改，只加 adapter + 特有表单。Slack / 企微 / 钉钉走 P2。

| 类型 | 公网 | 分期 |
|------|------|------|
| **飞书长连接** | 不需要 | **P1** |
| Slack Socket Mode | 不需要 | P2（群/频道按 §2.7：仅 @ 或引用，回复开 thread） |
| 企微回调 | 通常要 | P2（缺公网根可保存、`status=error`） |
| 钉钉 Stream | 不需要 | P2 |
| 公众号 | 要 | 更后 |
| 个微 | — | 不做 |

P1 范围：飞书**私聊**文本；共有启停/状态/查看会话；异步回复；幂等；忙则排队；**允许沙箱**；**不抽记忆**；**不**先发「还在处理」；**按通道实例的日志/用量**。不做群、富媒体、卡片交互。

### P2（已落地）

- 其余类型：`slack`（默认 Socket Mode）、`wecom`（HTTP 回调）、`dingtalk`（Stream）；`wechat_mp` 仅类型骨架 + 设置灰显「即将支持」。
- 群 / 频道：仅 @ 或引用才回；无 @ 裸消息忽略；有 thread 则回复进 thread；发送者白名单；群聊默认关沙箱。
- 富媒体走现有 `attachments`。
- webhook 缺 `public_base_url`：可保存，启用则 `status=error`（企微接上 P1 单测口径）。
- `needs_input`（ask_user / 沙箱确认）IM 侧先纯文本降级。

**P2 明确后置：** 类型特有卡片、实例级写库开关、删除实例与毁卷、「改为独立人设」、公众号完整接入。
...
7. 配好飞书字段并启用后，私聊文本能收到回复（人设为该实例所选）。
8. 改共用人员设，下一轮脚本通道与飞书通道都换新提示词。
9. 关实例或校验失败：外部消息进不来；已有时间线仍能「查看会话」。
10. 同一人连发两条：第二条不丢（排队），不双开同一 `role_id` 的 turn。
11. 左栏 tip 不出现该通道会话；**不跑记忆抽取**。
12. 写库 / 回写 / 改 Skill / `send_message` 不存在（mode=api）；**沙箱工具可用**。
13. 无 `public_base_url` 时飞书长连接仍能工作。
14. 脚本通道与飞书通道在列表里同级，共有「说话方式 / 查看会话 / 启停 / 状态」。
15. 长回合**不**先发「还在处理」；完成后一次写出站回复。
16. 用量/日志可按该通道实例查看（P1）。
17. webhook 类（即便 P1 未做）保存缺公网根的配置不得 400；启用则 `status=error` 且卡片说明原因——P1 可用单测钉死，P2 企微接上。

### P2

18. 设置列表可选 Slack / 企微 / 钉钉；微信公众号灰显「即将支持」，创建返回即将支持。
19. Slack 频道 / 飞书群 / 钉钉群：无 @、无引用的裸消息忽略；@ 或引用后开回合；Slack 命中后回复进 thread。
20. 群聊默认无沙箱工具，直到发送者在实例白名单。
21. 入站图片/文件落到现有「媒体/上传」attachments。
22. 企微缺公网根可保存；勾选启用则 `status=error` 且文案含公网根。
23. `ask_user` 在 IM 里变成带选项的纯文本，并提示到网页确认。
...
- 多实例共用一个 `role_id`。
- 页签主名或副名叫「开放接口」；页签只写「插件」。
- 跟随左栏角色提示词，或把通道挂到左栏 `role_id`。
- 按外部线程默认再开多把沙箱（策略 B 须另开开关，本文不包含）。
- 长回合先发「还在处理」。
- 群/频道里无 @、无引用的裸消息开回合；有 thread 能力时在顶栏刷回复。
- 非脚本通道 P1 抽取主人画像。
- 因缺 `public_base_url` 而禁止保存 webhook 配置。
- 个微 / 非官方协议。
...
| # | 现口径 | 已拍板的实现变更 | 为何 |
|---|--------|------------------|------|
| 1 | 设置入口叫「开放接口」，首页只管密钥 | 页签改为「聊天通道」；密钥是第一种通道实例，与 IM **同级列表**；不要「开放接口」副名 | §12 |
| 2 | 删人设只拦「仍有密钥」 | 改为「仍有通道实例」 | 共有人设 |
| 3 | 左栏/记忆排除只认 `origin=api` | 排除全部通道 origin；非脚本 P1 **不 dirty** | 飞书会话不能进 tip / 画像 |
| 4 | `is_api_role_id` 前缀 `api_` | hidden 通道工作角色含 `ext_` | 非脚本类型 |
| 5 | 同 Key HTTP 409 | **脚本保持 409**；其它类型 ACK+排队 | 平台会重试 |
| 6 | 记忆：API 不 dirty | 其它通道 P1 同样不抽 | §12 |
| 7 | 人设与左栏：只复制、不跟随 | **方案 A**，不跟随左栏提示词，不绑左栏 `role_id` | §12 |

不改：每通道实例一角色、人设可共享、mode=api 工具集、脚本同步 JSON+120s/202、不做多租户。

---

## 12. 已确认清单

主人于 2026-09-14 拍板。下列不再开放；实现按表执行。

| # | 问题 | 决定 |
|---|------|------|
| 1 | 页签名 | **「聊天通道」**。不要「开放接口」作主名，也不保留副名 |
| 2 | 说话方式 | 方案 **A**：只绑 persona；从左栏只复制副本；不跟随左栏角色提示词；不绑左栏 `role_id` |
| 3 | P1 第一家 IM | **飞书长连接** |
| 4 | 工作角色 | **每实例一角色 + IM 入站排队**（不按外部线程开角色） |
| 5 | 非脚本通道私聊记忆抽取 | **否**（P1 不抽） |
| 6 | 非脚本通道沙箱 | **允许**。群聊仍默认关沙箱，直到有发送者白名单（P2） |
| 7 | 长回合先发「还在处理」 | **不要** |
| 8 | Slack 频道无 thread 裸消息 | **频道/群仅被 @ 或被引用才开回合**；无 @ 裸消息忽略；有 thread 能力则**回复开成 thread**。私聊：每人一段会话。P1 仍以飞书私聊为主；此条为跨类型群/频道口径 |
| 9 | webhook 未配 `public_base_url` | **允许保存配置**；启用校验失败 → `status=error`，卡片写明缺公网根；不要禁止保存。长连接与 `script_api` 仍可不依赖公网 |
| 10 | 日志/用量按通道实例 | **P1** 再做。P0 脚本继续「上次调用」弱化展示 |

确认后不再开放：跟随左栏人设、按线程开角色、P1 抽记忆、长回合预告、禁止保存缺公网配置、P1 改做 Slack。
...
4. P1：飞书长连接 adapter + 特有表单 + 按实例日志/用量；不改公共模型。
